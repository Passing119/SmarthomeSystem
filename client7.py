...
# MQTT Broker 配置
BROKER_HOST = "10.200.10.219"  # Broker 地址
...
# 发布控制指令
def control_aircon(power=None, temperature=None, fan_speed=None, fan_direction=None, sd=None):
    ...
    if sd is not None:
        client.publish("sd", sd)
        print(f"发布湿度指令: {sd}")

# 示例：控制空调并发布湿度
control_aircon(power="on", temperature=25, fan_speed="high", fan_direction="auto", sd=51)
